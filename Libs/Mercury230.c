...
uint8_t GetCommandMsg(MERCURY_CMD_t command, MERCURY_ARRAY_NUM_t array_number, uint8_t address, uint8_t *buf, uint8_t *length)
{
	...
	switch(command)
	{
		case MERQURY_CMD_TEST:
			CRC = UpdCRC(buf_loc, 2);
			buf_loc[2] = CRC / 256;
			buf_loc[3] = CRC % 256;
			*length = 4;
			result = 1;
		...
		case MERQURY_CMD_OPEN:
			
		break;
		
		case MERQURY_CMD_CLOSE:
			
		break;
		
		case MERQURY_CMD_READ_ARRAY:
